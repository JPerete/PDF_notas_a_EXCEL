"""Programa para Pasar las notas en PDF ordenadas por orden alfabético a EXCEL ordenadas por nota"""
# Version 1
import re
import os
from datetime import datetime
from tkinter import Tk, ttk, Button, Label, filedialog, messagebox, _tkinter
import pdfplumber
import pandas as pd
...
class AplicacionOposicion:
    ...
    def __init__(self, root: Tk): # Incicializar la ventana
        ...
        # Selector de año
        Label(root, text="1. Selecciona el Año de la Convocatoria:",
              font=("Arial", 10, "bold")).pack(pady=(15, 5))
        ano_actual = datetime.now().year
        anyos_disponibles = [str(año) for año in range(ano_actual, ano_actual - 15, -1)]

        self.combo_anos = ttk.Combobox(root, values=anyos_disponibles,
                                       state="readonly", font=("Arial", 10), width=15)
        self.combo_anos.set(str(ano_actual))
        self.combo_anos.pack(pady=5)

        # Selector de PDF
        Label(root, text="2. Selecciona el archivo PDF de la lista:",
              font=("Arial", 10, "bold")).pack(pady=(15, 5))
        self.btn_buscar = Button(root, text="Buscar PDF...",
                                 command=self.buscar_pdf,
                                 font=("Arial", 10), bg="#2196F3", fg="white")
        self.btn_buscar.pack(pady=5)

        # Etiqueta dinámica que mostrará el nombre del archivo seleccionado
        self.lbl_archivo = Label(root, text="Ningún archivo seleccionado",
                                 font=("Arial", 9, "italic"),
        ...
        self.btn_procesar = Button(root, text="Generar Excel Ordenado",
        ...
        self.lbl_estado = Label(root, text="",
                                font=("Arial", 10, "italic"),
                                fg="#002244")
        self.lbl_estado.pack(pady=5)

        # Créditos / Disclaimer al pie de la ventana
        self.lbl_creditos = Label(root, text="Aplicación desarrollada por Juan Perete Pinar" \
                                             " con ayuda de la IA",
                                  font=("Arial", 8, "italic"), fg="gray")
        self.lbl_creditos.pack(pady=(10, 0))
        self.lbl_contacto = Label(root, text="Contacto: juan.peretepinar@gmail.com",
                                  font=("Arial", 8, "italic"), fg="gray")
        self.lbl_contacto.pack(side="bottom", pady=(2, 10))

    def buscar_pdf(self):
        """Abre el explorador de archivos y actualiza el texto de la interfaz."""
        ruta = filedialog.askopenfilename(title="Selecciona el PDF de la oposición",
                                          filetypes=[("Archivos PDF", "*.pdf")])


        # Usamos el operador ternario para acortar el texto si el nombre es muy largo
        if ruta:
            self.ruta_pdf_seleccionado = ruta
    ...
    def ejecutar_proceso(self):
        """Lanza el proceso y muestra un mensaje de éxito o error al terminar."""
        ...
        if exito:  # Cierra la aplicación por completo al terminar con éxito
            ...
            # Abre el Excel con el programa predeterminado de Windows
            os.startfile(ruta_excel)

            messagebox.showinfo("¡Éxito!",
                                f"El archivo Excel se ha creado correctamente en:\n\n{ruta_excel}")
            self.root.destroy()  # Cierra la aplicación
        else:
            self.lbl_estado.config(text="Error en el procesamiento",
                                   fg="red")
            messagebox.showerror("Error",
                                 "No se han podido extraer datos válidos del PDF.\n" \
    ...
    @staticmethod
    def extraer_datos_columna(texto_columna):
        """Procesa el texto línea por línea para extraer DNI, Nombre y Nota."""
        filas = []
        ...
        for linea in lineas:
            ...
            # FILTRO: Si la línea contiene la palabra "Registros" (o similares),
            # saltamos a la siguiente (para que no procese los piés de página)
    ...
    @staticmethod
    def procesar_oposicion(ruta_pdf, ano_convocatoria):
        """Realiza la lectura del PDF y genera el archivo Excel ordenado."""
